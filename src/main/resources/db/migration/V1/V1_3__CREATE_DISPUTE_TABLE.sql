-- Optional ENUM or just use VARCHAR with CHECK constraint for DisputeState
-- If you prefer to use ENUM:
-- CREATE TYPE dispute_state AS ENUM ('NEW', 'INVESTIGATING', 'RESOLVED', ...);
...
CREATE TABLE disputes (
    ...
    -- a combination of these will uniquely identify the disputant
    disputant_id BIGINT NOT NULL,
    disputant_type VARCHAR(50) NOT NULL CHECK (
        disputant_type IN ('CUSTOMERUSER', 'INSTITUTIONUSER', 'INTERNALUSER')
    ),
    disputant_domain_code NOT NULL,
