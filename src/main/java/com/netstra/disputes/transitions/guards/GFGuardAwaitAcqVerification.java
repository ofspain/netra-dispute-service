...
public class GFGuardAwaitAcqVerification implements Guard<DisputeState, DisputeTransitionEvent> {
    ...
    public boolean evaluate(StateContext<DisputeState, DisputeTransitionEvent> context) {
        ...
        switch (target){
            case ACQUIRER_VERIFIED -> {

            }
            case AWAITING_ACQUIRER_VERIFICATION_UNREACHABLE -> {

            }
            case ACQUIRER_DECLINED -> {

            }
            case WITHDRAWN_CUSTOMER, WITHDRAWN_SUB_INSTITUTION, WITHDRAWN_ISSUER -> {
